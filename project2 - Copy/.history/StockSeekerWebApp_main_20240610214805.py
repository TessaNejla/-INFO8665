...
date_range_picker = pn.widgets.DateRangePicker(name='Select Date Range', start=datetime(2020, 1, 1), end=datetime.now())
...
# Function to run analysis on button click
def run_analysis(event):
    selected_tickers = ticker_selector.value
    start_date, end_date = date_range_picker.value
    analysis_type = analysis_type_selector.value
    selected_options = {key: widget for key, widget in additional_info.items() if widget.value}
    
    if selected_tickers and start_date and end_date:
        # Create a list of analysis results
        analysis_results = []
        for ticker in selected_tickers:
            stock_analysis = display_stock_analysis(ticker, analysis_type, start_date, end_date)
            additional_info_tabs = display_additional_information(ticker, selected_options)
            
            stock_pane = pn.Column(pn.pane.Plotly(stock_analysis), additional_info_tabs)
            analysis_results.append(stock_pane)
        
        # Display the analysis results
        template.main[:2] = pn.Column(*analysis_results)

# Attach the button click event to the analysis function
