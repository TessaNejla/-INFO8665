...
# Function to load local CSS# Load custom CSS for styling
css_file_path = r"C:\Users\Admin\Documents\MLAI\-INFO8665\project2\docs\assets\style.css"   # Update the path as needed
...
analysis_type_select = pn.widgets.Select(name='Select Analysis Type', options=["Closing Prices", "Volume", "Moving Averages", "Daily Returns", "Correlation Heatmap", "Distribution of Daily Changes"])

additional_info = pn.widgets.MultiChoice(name='Display Additional Information', options=["Stock Actions", "Quarterly Financials", "Institutional Shareholders", "Quarterly Balance Sheet", "Quarterly Cashflow", "Analysts Recommendation", "Predicted Prices"])
...
def display_predicted_prices(selected_stock, start_date, end_date):
    ...
    table = valid[['Close', 'Predictions']].tail(10)
    
    return pn.Column(pn.pane.Plotly(fig), pn.widgets.DataFrame(table.reset_index()))
#Main App Logic
...
def analyze_button_click(event):
    ...
    for selected_stock in ticker_select.value:
        stock_analysis_pane.append(display_stock_analysis(selected_stock, analysis_type_select.value, start_date_picker.value, end_date_picker.value))
...
main_layout = pn.Column(
    ...
    pn.Row(
        pn.Column(ticker_select, start_date_picker, end_date_picker, analysis_type_select, analyze_button),
        pn.Column(additional_info, summary_button)
    ),
    pn.Tabs(
        ("Stock Analysis", stock_analysis_pane),
        ("Additional Information", additional_info_pane)
    )
)
